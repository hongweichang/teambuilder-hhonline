...
<div id="cate_title_block">
	...
	<asp:Repeater ID="repCategories" runat="server">
		<HeaderTemplate>
			<ul id="cate_item">
				<li id="cate_item_parent"><a class="cate_link" href="view.aspx?news-newslist">资讯中心</a></li>
		</HeaderTemplate>
		<ItemTemplate>
			<li id="cate_item_<%#Eval("ID") %>" onmouseover="cateShow(<%#Eval("ID") %>)" onmouseout="cateHidden(<%#Eval("ID") %>)">
				<a class="cate_link" href="view.aspx?news-newslist&cate=<%# HHOnline.Framework.GlobalSettings.Encrypt(Eval("ID").ToString()) %>">
					<%#Eval("Name") %></a></li>
		</ItemTemplate>
		<FooterTemplate>
			</ul>
