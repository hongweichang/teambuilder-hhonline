...
function renderDataToClient(data) {
    ...
    if (data.cats != '') {
        ...
        $(data.cats).each(function(i, n) {
            pId = n.ParentID;
            sb.append(htmlTemp.format(n.IndustryID, n.IndustryName.cut(16), n.IndustryName));
        });
        fp.html(sb.toString());
    }
    ...
    if (data.nav != '') {
        ...
        $(data.nav).each(function(i, n) {
            if (i == 0) {
                html += ('<span title="{0}">{1}</span>').format(n.IndustryName, n.IndustryName.cut(15));
            }
            else {
                html = ('<a title="{2}" onclick="callServer({0});" href="javascript:{}">{1}</a>&gt;&gt;').format(n.IndustryID, n.IndustryName, n.IndustryName.cut(15)) + html;
            }
        });
...
function insertCats() {
    ...
    fp.find('input[checked]').each(function() {
        var m = $(this);
        parent.addIndustries(m.val(), m.attr('text'));
    });
